...
import spinal.lib._
import conv.wa.xip.memory.xpm.{CLOCK_MODE, MEM_TYPE, sdpram}
import conv.wa.{WaCounter, setClear}
...
case class LoadWeight(convConfig: ConvConfig) extends Component {
    ...
    val copyWeightCnt = WaCounter(fsm.currentState === LoadWeightEnum.COPY_WEIGHT && io.sData.fire, log2Up(convConfig.WEIGHT_S_DATA_DEPTH), io.weightNum - 1) //一个点权重要入多少次
    val copyWeightTimes = WaCounter(copyWeightCnt.valid, convConfig.KERNEL_NUM.toBinaryString.length, convConfig.KERNEL_NUM - 1) //9个点全部入完
    val channelInCnt = WaCounter(fsm.currentState === LoadWeightEnum.COPY_WEIGHT && io.sData.fire, channelInTimes.getWidth, channelInTimes - 1) // 一个点入8通道就换另一个卷积核了，所以计一个点入完所有通道要多少次  ??可能不大对
